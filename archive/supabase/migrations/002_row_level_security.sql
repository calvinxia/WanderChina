-- ============================================================================
-- WanderChina Row Level Security (RLS) Policies
-- Version: 1.0.0
-- Created: 2025-10-23
-- Platform: Supabase
-- ============================================================================
...
-- Only admins can modify places (handled by service role)
...
-- Only backend can insert achievements (via service role)
...
CREATE POLICY "Users can view seeking profiles"
    ...
    TO authenticated
    USING (is_seeking_companions = true AND verification_status = 'approved');
...
CREATE POLICY "Guides can view their bookings"
    ...
    USING (
        EXISTS (
            ...
            AND local_guides.user_id = auth.uid()
        )
    );
...
-- Recipients can view location shares (by share_code)
-- This is handled in application logic with share_code validation
...
CREATE POLICY "Users can view own location updates"
    ...
    USING (
        EXISTS (
            ...
            AND location_shares.user_id = auth.uid()
        )
    );
