...
class MapTranslationService {
  ...
  Future<String?> _queryRoadFromDatabase(String roadName, String? city) async {
    try {
      final query = _db.from('road_translations').select('road_name_en');

      query.eq('road_name_zh', roadName);
      ...
      if (result != null) {
        // 更新使用次数
        await _db.from('road_translations')
            .eq('road_name_zh', roadName)
            .update({'usage_count': result['usage_count'] ?? 0 + 1});

        return result['road_name_en'];
      }
  ...
  Future<String?> _queryStationFromDatabase(
    ...
    try {
      final result = await _db
          ...
          .eq('station_name_zh', stationName)
          .eq('city', city)
      ...
      if (result != null) {
        // 更新使用次数
        await _db.from('transit_station_translations')
            .eq('station_name_zh', stationName)
            .eq('city', city)
            .update({'usage_count': result['usage_count'] ?? 0 + 1});

        return result['station_name_en'];
      }
  ...
  Future<String?> _queryInstructionFromDatabase(String instruction) async {
    try {
      final result = await _db
          ...
          .select('instruction_en')
          .eq('instruction_zh', instruction)
          .single();

      if (result != null) {
        return result['instruction_en'];
      }
  ...
  Future<String?> translateArea(String areaName, {required String city}) async {
    ...
    try {
      ...
      final result = await _db
          ...
          .select('area_name_en')
          .eq('area_name_zh', areaName)
          .eq('city', city)
          .single();

      if (result != null) {
        final translation = result['area_name_en'];
        _areaCache[cacheKey] = translation;
        await _saveCacheToStorage('area', cacheKey, translation);
        return translation;
      }
